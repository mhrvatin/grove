...
extracted: no `@facit` or internal deps, all project-specific facts isolated in one
`grove.config.ts`. The launcher CLIs stay on `node:*`/Bun built-ins; the dashboard is
...
- **Standalone repo / npm publish.** Already extraction-ready (own `package.json`,
  no `@facit`/internal deps — lift the directory out); pay the multi-repo tax only
  when a second consumer exists. The dashboard depends on React/Vite (DASH-18); the
...
| ID | Status | Requirement |
|----|--------|-------------|
| DASH-1 | Superseded by DASH-1a | ~~`just grove` SHALL start a `Bun.serve` HTTP dashboard bound to `127.0.0.1` only, detached in the background. It SHALL be idempotent — if already listening on its port, just print the URL.~~ |
| DASH-1a | Done | `just grove` SHALL start a `Bun.serve` HTTP dashboard bound to `127.0.0.1` only, detached in the background. It SHALL be idempotent — if already listening on its port it is a silent no-op. `start`/`stop` SHALL be quiet on success (the URL is the fixed `localhost:PORT`); only a failure surfaces, as a single error line. |
...
| DASH-18 | Done | The dashboard SHALL be a React + Vite + TypeScript single-page app (the build-step / framework deferral DASH-17 carried from DASH-7 is intentionally dropped — see the standalone-repo non-goal). It SHALL build to `dist/` and be served as static assets by the same `Bun.serve` that exposes the JSON API, with any non-`/api` path that is not a real file falling back to `dist/index.html` (one screen, no client router). The build SHALL run on `just grove` *inside* `start()`, **after** the already-running no-op check (DASH-1a), so a running dashboard never rebuilds; `start()` runs `vite build` only (a type error SHALL NOT block viewing the dashboard — the full typecheck lives in `bun run build` / CI). The brand stylesheet (`dashboard.css`) SHALL be reused verbatim as the DOM/style contract (status dots, 680px stacking, log drawer) — imported, not re-authored. Grove SHALL take no `@facit`/internal deps and copy nothing from `packages/web`; its React/Vite deps SHALL be exact-pinned. |
