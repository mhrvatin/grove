...
| DASH-17 | Done | The dashboard's static assets SHALL live in real files — the stylesheet in `dashboard.css`, the client poll/morph script in `dashboard.client.js`, and the page skeleton in `dashboard.html` (with placeholders the server fills) — served by the same `Bun.serve` (the HTML references the CSS/JS by URL). The server source SHALL NOT embed the stylesheet or client script as an inline string literal. Per-request *dynamic* rendering — the `/rows` fragment (`renderRows`) and the header counts — legitimately stays in server code. This supersedes only DASH-7's "multiple files" deferral: a build step, bundler, transpiler, and component framework remain **deferred** — splitting files SHALL NOT introduce any of them; grove stays on `node:*`/Bun built-ins only (per the extraction non-goal). |
