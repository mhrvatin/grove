...
| DASH-19 | Done | `GET /api/meta` SHALL return `{ repoName }`, the basename of the main repo root (PORT-5's repo identity, DASH-20's `dist/` is shared across repos so only the running server process knows which one it's serving). The header wordmark SHALL render `Grove - <repoName>` (falling back to plain `Grove` before the fetch resolves). The background gradient's hue SHALL be hashed from `repoName` so two dashboards for different repos are visually distinguishable. |
