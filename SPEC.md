...
a React + Vite + TypeScript SPA with its own pinned dependencies (DASH-18), built to
`dist/` and served by the same `Bun.serve`.
...
| DASH-18 | Done | The dashboard SHALL be a React + Vite + TypeScript single-page app (the build-step / framework deferral DASH-17 carried from DASH-7 is intentionally dropped — see the standalone-repo non-goal). It SHALL build to `dist/` and be served as static assets by the same `Bun.serve` that exposes the JSON API, with any non-`/api` path that is not a real file falling back to `dist/index.html` (one screen, no client router). The build SHALL run on `grove start` *inside* `start()`, **after** the already-running no-op check (DASH-1b), so a running dashboard never rebuilds; `start()` runs `vite build` only (a type error SHALL NOT block viewing the dashboard — the full typecheck lives in `bun run build` / CI). The brand stylesheet (`dashboard.css`) is the DOM/style contract (status dots, 680px stacking, log drawer) — imported, not re-authored. Grove SHALL take no external project deps; its React/Vite deps SHALL be exact-pinned. |
| DASH-19 | Done | `GET /api/meta` SHALL return `{ repoName }`, the basename of the main repo root (PORT-5's repo identity, DASH-18's built `dist/` is shared across repos so only the running server process knows which one it's serving). The header wordmark SHALL render `Grove - <repoName>` (falling back to plain `Grove` before the fetch resolves). The background gradient's hue SHALL be hashed from `repoName` so two dashboards for different repos are visually distinguishable. |
