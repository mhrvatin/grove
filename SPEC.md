...
| DASH-1b | Done | `grove start` SHALL start a `Bun.serve` HTTP dashboard bound to `127.0.0.1` only, on the port from PORT-5, detached in the background. It SHALL be idempotent — if already listening on its port it is a no-op — but since the port is no longer a fixed well-known value, `start` SHALL always print `dashboard on http://localhost:<port>` on both the fresh-start and the already-running no-op path. `stop` SHALL stay quiet on success; only a failure surfaces, as a single error line. |
...
| DASH-19 | Done | `GET /api/meta` SHALL return `{ repoName }`, the basename of the main repo root (PORT-5's repo identity, DASH-20's `dist/` is shared across repos so only the running server process knows which one it's serving). The header wordmark SHALL render `Grove - <repoName>` (falling back to plain `Grove` before the fetch resolves), and the browser tab title (`document.title`) SHALL be set to the same `Grove - <repoName>` string once the fetch resolves. The background gradient's hue SHALL be hashed from `repoName` so two dashboards for different repos are visually distinguishable. |
