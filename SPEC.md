...
| DASH-1a | Done | `grove start` SHALL start a `Bun.serve` HTTP dashboard bound to `127.0.0.1` only, detached in the background. It SHALL be idempotent — if already listening on its port it is a silent no-op. `start`/`stop` SHALL be quiet on success (the URL is the fixed `localhost:PORT`); only a failure surfaces, as a single error line. |
...
| DASH-18 | Done | The dashboard SHALL be a React + Vite + TypeScript single-page app (the build-step / framework deferral DASH-17 carried from DASH-7 is intentionally dropped — see the standalone-repo non-goal). It SHALL build to `dist/` and be served as static assets by the same `Bun.serve` that exposes the JSON API, with any non-`/api` path that is not a real file falling back to `dist/index.html` (one screen, no client router). The build SHALL run on `grove start` *inside* `start()`, **after** the already-running no-op check (DASH-1a), so a running dashboard never rebuilds; `start()` runs `vite build` only (a type error SHALL NOT block viewing the dashboard — the full typecheck lives in `bun run build` / CI). The brand stylesheet (`dashboard.css`) is the DOM/style contract (status dots, 680px stacking, log drawer) — imported, not re-authored. Grove SHALL take no external project deps; its React/Vite deps SHALL be exact-pinned. |
